...
namespace RavenM
{
    ...
    [HarmonyPatch]
    public class PhotoModePatch
    {
        [HarmonyPatch(typeof(FpsActorController), "EnablePhotoMode")]

        static void EnablePhotoMode(FpsActorController __instance)
        {
            if (!IngameNetManager.instance.IsClient)
                return;
        ...
        static void DisablePhotoMode(FpsActorController __instance)
        {
            if (!IngameNetManager.instance.IsClient)
                return;
        ...
        [HarmonyPatch(typeof(FpsActorController), "TogglePhotoMode")]
        [HarmonyPrefix]
        static bool TogglePhotoMode()
        {
            if (LobbySystem.instance.InLobby)
                return LobbySystem.instance.EnableGodInspect || FpsActorController.instance.inPhotoMode;
            return true;
        }
    }

    [HarmonyPatch]
    public class GodInspectPatch
    {
        [HarmonyPatch(typeof(AiActorController), "OnGUI")]
        [HarmonyPrefix]
        static bool OnGUI()
        {
            if (LobbySystem.instance.InLobby)
